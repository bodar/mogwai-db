...
| repeat BODY vocabulary | ✅ | any ROW-LOCAL body: `out/in/both/…E/…V/otherV` chains, `has`/`hasLabel`/`hasId`/`where`/`filter`/`not`/`and`/`or`, a uniform-element branch (`union`/`choose`/`coalesce`/`optional`), and a NESTED `repeat`. The flat frontier expansion stays the fast path for movement+`has()`; anything else compiles the body ONCE through the ordinary StepFns as a KEYED CHILD RELATION the recursive term joins (legal because a recursive term may reference a non-recursive CTE — the way around SQLite having no `LATERAL`). ❌ a PER-ITERATION GLOBAL barrier (`dedup`/`order`/`limit`/`range`/`sample`/`tail`/`group`/`aggregate`/`local`): it observes the whole frontier at one iteration, so precomputing it per-origin would answer a different question — a fixed `times(n)` could be unrolled into n generic phases instead (not built). ❌ a label bound INSIDE the body (rebinds per iteration); a body that MENTIONS an outer label (the keyed domain is every vertex, so there is no outer row to read the alias column from — declined rather than answered empty); a path with a multi-hop body |
