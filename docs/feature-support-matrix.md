...
| `coalesce(…)` | 🟡 | ✅ first-non-empty via the `St.origins` ordinal STACK for element arms and homogeneous scalar projection tails with scoped `count/sum/min/max/mean`; later scalar steps compose<br>✅ **incoming `as()` threads through** (originSeed projects it alongside the ordinal, merge preserves it)<br>✅ **nested in coalesce/optional** for element arms (each branch pushes a unique ordinal `o0`/`o1`/…)<br>❌ mixed-shape<br>❌ a NEW `as()` inside an arm<br>✅ **path threads through** element arms (pad-to-max cols) |
| `union(…)` | 🟡 | ✅ element multi-hop arms via `foldBody`; homogeneous scalar projection tails with scoped `count/sum/min/max/mean` lower through child ScalarStreams + `UNION ALL` (dynamic numeric `v,vt` retained), and later scalar steps compose<br>✅ **incoming `as()` threads through** the merge (`mergeBranchCarried`), so `union(…).select('a')`/`.path()` resolve<br>❌ mixed-shape<br>❌ source-branch tails<br>❌ a NEW `as()` inside an arm<br>✅ **path threads through** element arms (pad-to-max cols) |
