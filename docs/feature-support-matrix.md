...
| `select('a')`, multi-`select`, `project(…)` | 🟡 | ✅ column-threaded aliases<br>✅ single-label `select` retypes to an element (vertex/edge) or ScalarStream under `by(key)`; later steps compose<br>✅ multi-`select`/`project` lower to a heterogeneous per-traverser RecordStream (scalar/vertex/**edge** fields); selecting a field re-enters ordinary scalar/element lowering, and `count()` composes<br>✅ `project(...).by(__.<scalar child>)`: every field uses generic child-first productivity joined by a multiset-safe outer origin; productive NULL survives<br>✅ `limit`/`range`/`skip`/`tail` with `Scope.local` slice record fields<br>❌ mixed traversal/string project modulators; traversal-valued select; record `order`/`dedup`/`fold`/`where` |
