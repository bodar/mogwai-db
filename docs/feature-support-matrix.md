...
no note, the whole step works. **L3 conformance: <!-- L3:passing -->1,810<!-- /L3:passing --> · corpus parse+chain: 2298/2298.**
...
where a user will see it, not only in a tracker. L5 (`test/L5-properties/`) found four such forms —
it was the first thing ever to execute the compiler's generic lowering path — and all four are fixed.
...
| `io(path).read()` / `.write()` | 🟡 | desugars to `call("mogwai.io", …)` (one canonicalize Pass), so it inherits the barrier seam and the compiler learns no second async step kind. Typed GraphSON (`.json`) both ways, lossless over the whole type channel; **Neptune/Neo4j CSV (`.csv`) both ways for INTEROP** — reads either dialect's typed header, writes Neptune, and a write emits the vertex/edge PAIR at `<stem>-vertices`/`<stem>-edges` (one header cannot describe both). `.with(IO.reader\|IO.writer, …)` overrides the extension. Documents live behind `IoStore` — a rooted directory on Bun (`$MOGWAI_IO_DIR`), an R2 binding in a DO. ❌ GraphML (`.xml`) and Gryo (`.kryo`), both fail closed naming the format; no binding fails closed naming the binding; a CSV export REFUSES a collection-valued property or a meta-property (no column can carry either) and declares `bigint`/`bigdecimal`/`uuid`/`char`/`duration` as `String`, so they read back as strings |
...
| `and`, `or`, `not`, `filter(__.…)` | ✅ | incl. infix `.and()`/`.or()` on the STEPS **and on PREDICATES** — `has('name', TextP.startingWith('m').or(TextP.startingWith('p')))`, `has('age', P.gt(20).and(P.lt(30)))`, `P.gt(30).negate()` — composing to any depth and in every predicate host (`has`/`where`/`filter`/`not`/`is`). ❌ `filter(rawPredicate)` — use a traversal |
...
| `properties(k…)` [`.key`/`.value`/`.element`/`.id`/`.label`/`.count`] | ✅ | full property stream with owner/key/value/meta, all re-enterable; `dedup()` keeps VertexProperty identity (`vpid`) and edge key/value identity; `dedup().by(value)` deduplicates property values; `order()` supports natural order, `by(T.key)`, `by(T.value)`, direction-only `by(asc/desc)`, and one scalar traversal `by(__.value())`/`by(__.element().values(k))`, with typed value sorting and stable property identity ties; a property is also a `by()` HOST, so `group()` over a property stream and `by(__.key())`/`by(__.value())`/`by(__.element()…)` bodies compose wherever a child body is legal. ❌ nested property order after `as()`/path tracking; multi-term/traversal-valued `by()` modulators; fan-out traversal ordering beyond first emitted value |
...
| `order()` [`.by(key[,dir]\|__.trav)`] | ✅ | at the tail/root of an element stream: `by(key)` and `by(__.traversal)`, including **multi-term** orders mixing property keys, `T.label`/`T.id` tokens, and traversal terms — each traversal term computes its per-traverser sort column through the shared multi-modulator seam (`tryCompileScalarModulations`) and the composite `ORDER BY` combines them round-robin. A sort MINTS the emission position on both element and value streams (an order that is not a column cannot survive a relation boundary), so `order()` composes with a following `tail()`, slice, movement or retype. ❌ after `path()`/encounter tracking; `by(key)` on a scalar stream; shuffle mixed with other terms; a traversal-valued `by()` on an `order()` INSIDE a `where()`/existence child or a `dedup().by()` barrier (the correlated `elementOrderSql` path — key/direction only there). **Non-productive `by(key)` drops** traversers lacking the key (TinkerPop's default; `ProductiveByStrategy` keeps them, nulls first) — one policy (`orderProductivityFilter`, `tail/modulation.ts`) applied at each element-order route (terminal render, scalar projection, order-then-movement re-enter, child body), sharing `classifyBy` with the sort terms so the filter and the ORDER BY cannot disagree about which by()s project a key. An `order()` followed ANYWHERE by a movement/branch/re-source retypes to an ordered element stream and re-enters, so `order().by(k).limit(n).out(l)` composes (the gate used to look only at the IMMEDIATE follower, and a slice hid the movement behind it). A folded `order().by(k)` before a `dedup()` applies BOTH drops — its own and the dedup's `by()` — two different keys on two different steps |
| `limit`, `range`, `skip`, `tail`, `sample` | ✅ | mid-chain or tail; `Scope.local` slices record fields. After a fan-out they pick a DETERMINISTIC subset via the canonical emission order (a demand pre-pass seeds it; order-free chains stay order-free). `tail(n)` is the same window read backwards; `sample(n)` is a uniform sample (`ORDER BY RANDOM()`). Under `movementCollapse` a slice counts TRAVERSERS, not rows — the boundary row's multiplicity is trimmed. ❌ `tail` after a barrier has consumed the emission order; a WEIGHTED `sample().by()` |
...
| Step | | Notes |
|---|:--:|---|
| `group`, `groupCount` | ✅ | scalar/`T.id`/`T.label`/composite-`project` keys; scalar reducers, element values, unreduced value traversals, and nested-map values (`by(__.<movement>.group())`) all compose through the generic child seam; group-scoped `count/sum/min/max/mean`/`fold()` — a value body composes a scalar PROJECTION with EVERY reducer uniformly (`by(__.label().count())`, `by(__.values('k').count())`, `by(__.out().values('k').count())`), the two shape classifiers being complementary rather than alternative; scalar-stream `groupCount()`; `unfold()`/`select(Column.keys/values)`/`order(Scope.local).by(Column.keys/values[, Order])` re-enter the map (via the whole-map blob stream; the local order re-sorts the pairs array in place, type-correctly via `compareKey`, and composes with a following `unfold()`/`select`); a **terminal** `select(Column.values)` over an element-LIST value frames the full vertices/edges (the nested-element-list framing recurses through `listResult`/`frameListOf`, matching the `.unfold()` variants); a **RECORD key** (`by(__.project('o','l','i')…)`) at any depth, with each field keeping its own encoding — an ELEMENT field frames as a vertex/edge, a stored value keeps its type, an unproductive field omits its key, and a nested `project()` nests; an ELEMENT-valued key part reached by re-rooting the host (`by(__.outV())`, `by(__.element())` — exactly one traverser by the schema); the barrier also runs over a **PROPERTY stream**, whose members are the VertexProperties themselves. ❌ more than two `by()`; a LIST-valued key part; `order().by(key)` inside a value; `order(Scope.local)` over an element/list-valued map side; `select(Column)`/`unfold()` over a nested-map value; a group-scoped REDUCER value (`by(__.out().count())`) over a property host |
...
| `until(<pred>)` / `emit(<pred>)`, `loops().is(n)` | ✅ | do-while / while-do; `emit(pred)` in either position. **Two routes for the predicate BODY:** the inline correlated compiler is tried first (it alone reads the walk's per-iteration state — `loops()`, the sack), and anything it declines that is ROW-LOCAL compiles ONCE over every vertex as a keyed relation whose origin set the recursive term tests with `id IN …` (until/emit are existence, so that is the whole semantics). So a branch predicate (`until(__.union(__.out(),__.in()))`, `coalesce`) now compiles. ❌ `until(__.loops()…)` beyond `loops().is(P)`; a predicate that is BOTH per-iteration and beyond inline lowering (e.g. `until(__.out().order().limit(1))`) |
| `repeat().path()`, `simplePath()` in body | ✅ | JSONB array walk + `json_each` cycle guard. **Every `by()` form answers the same in both regimes.** `by(key)`/`by(T.id)`/`by(T.label)` share ONE position projector (parameterized by how the element is reached — a joined table vs a correlated read off the exploded id), and `by(traversal)` runs the SAME positional child compiler as the linear regime: the `json_each` explode carries `(pk, ord)` as its `origins`, which makes it an ordinary element stream, so the fan-out guards, the `choose`/`coalesce` branch route and the `first` collapse are reused rather than reimplemented. Non-productive drop and ProductiveBy hold for every form. ❌ multiple `by()`s (round-robin needs a known length; a recursive path's is dynamic) |
| repeat BODY vocabulary | ✅ | **Row-local body vocabulary:** `out/in/both/…E/…V/otherV` chains, `has`/`hasLabel`/`hasId`/`where`/`filter`/`not`/`and`/`or`, a uniform-element branch (`union`/`choose`/`coalesce`/`optional`), and a NESTED `repeat`. The flat frontier expansion stays the fast path for movement+`has()`; anything else compiles the body ONCE through ordinary StepFns as a KEYED CHILD RELATION the recursive term joins. **Fixed-`times(n)` barrier route:** not built yet; `dedup`/`order`/`limit`/`range`/`sample`/`tail`/`group`/`aggregate`/`local` can be expressed as n ordinary phases by RelIR unrolling. **Platform wall:** an `until()`/`emit()` walk with a per-iteration global barrier cannot be expressed by SQLite's recursive term, whose barriers are not per-frontier. ❌ labels bound inside the body, outer-label reads, and multi-hop path bodies remain separate keyed-domain limitations. |
...
| `times(pred)`, `until`+`times`/`emit` | ❌ | combined exit forms (`until` together with `times`/`emit` fails closed); `times(pred)` does not parse |
...
| Step | | Notes |
|---|:--:|---|
| `match(p1, p2, …)` | 🟡 | conjunctive `as(start).<traversal>.as(end)` patterns, dependency-ordered, over the shared lowering (prefix fold + the shaped loop — `lowerRootedArm`'s shape, seeded from a bound var). A variable holds **any shape**: an element, an EDGE (and an edge var can then START a pattern — `as("a").outE().as("e"), as("e").inV().as("b")`), or a SCALAR (`values("name").as("b")`, type-tagged so a numeric var returns a number). Re-using a var constrains it, by rowid or by value per its shape. A REDUCER pattern (`count`/`sum`/`min`/`max`/`mean`) binds **per binding**, routed through the child seam — verified equal to `map(__.<same body>)` — so a global barrier no longer walls the pattern off. A `repeat()` walk composes as a pattern body. FILTER arguments — `not(…)`, `where(traversal)`, `where(var,P)` — bind nothing and constrain the binding table, delegated to the same `where` StepFn the engine routes them to, and scheduled once every variable they READ is bound (so argument order does not change the answer). Their labels canonicalize to `select`/`where(P.eq)` exactly as in a `where()` body — left as binds they would re-bind and, negated, drop every row. **Zero root variables** is legal: when every pattern start is already bound before the `match()`, nothing is seeded and `id` stays the incoming traverser. Labels bound inside the patterns are visible to a downstream `where(__.as("c")…)` and to sibling arguments (the conjunction is one scope). ❌ a bare `match()` as a TERMINAL (it must emit one binding **Map** per row; every supported form ends in `select(…)`); a LIST-shaped end var (`fold()`); a pattern STARTING from a non-element var (no rowid to re-root on); a re-bind at a different shape; `and`/`or` pattern GROUPS (they BIND their nested ends, so a filter lowering would answer a narrower question) and nested-match; more than one root var and a true mutual cycle (no pattern can go first); `dedup(label)` downstream; path tracking |
| `match("MATCH …")` — the GQL pattern STRING | ✅ | the whole of `MatchString.feature` (25/25). A second LANGUAGE in a string argument, parsed by its own generated parser (`parser/gql/`, from upstream's `GQL.g4`) and translated to the ordinary `match()` IR by one `extract` Pass, so the compiler learns no GQL concept. Node/edge variables, labels, inline `{k: v}` property filters on either, `$param` references (from the `match(str, [k: v])` map), directed/reverse/undirected edges, `-[]->` with no label, anonymous `()` nodes at any position, multiple comma-separated path patterns joined on a shared variable, cyclic patterns (a back edge CONSTRAINS rather than re-binds), `{k: null}` meaning the property is ABSENT, and typed literals sharing the Gremlin front end's own numeric typing. A TERMINAL one emits the binding **Map** (`select`/`project` over the declared variables — anonymous nodes never appear). Mid-traversal (`g.inject(1).match(…)`) re-sources per traverser. ❌ an undirected edge carrying a VARIABLE (the far endpoint has no entering-vertex context); a terminal pattern declaring no variables; `__.match("…")` in a nested position; `WHERE`/`RETURN`/path quantifiers (outside upstream's grammar) |
...
| set-ops (`combine`/`intersect`/`difference`/`disjunct`/`product`/`merge`/`conjoin`) | ✅ | operand = a literal list, `constant(c).fold()`, or a standalone scalar-fold traversal. The four SET results (`intersect`/`difference`/`disjunct`/`merge`) NAME their member order (`ORDER BY value`) rather than inheriting the dedup implementation's — `UNION` sorts and `SELECT DISTINCT` does not, so the two were different accidents for one concept. ❌ an element-fold operand |
...
| Step | | Notes |
|---|:--:|---|
| `addV()`, `.property(k,v)`, `property(T.id/T.label)` | ✅ | user-supplied ids; inline property VALUES + nested-traversal LABEL + constant nested KEY; over an ELEMENT or a SCALAR stream alike (`g.inject(1,2,3).addV(…)` creates three), since a creation's input is a MULTIPLIER. ❌ a live-read nested property KEY |
...
| `mergeV`, `mergeE` | 🟡 | id-aware upsert, onCreate/onMatch, start + mid-chain, and over a SCALAR stream (`g.inject(0).mergeV(…)`) — a creation's input is a MULTIPLIER, so its traverser need not be an element; map label/id/VALUES may be nested traversals; prop VALUES keep their type; a `T.label` on `onMatch` is append-only `addLabel` (an empty collection is a no-op); the SEARCH reads the merge map alone, so an endpoint or label that exists only on `option(onCreate)` narrows nothing; an ENDPOINT-LESS `mergeE` searches every edge and raises "Out Vertex not specified" only for the rows that matched none. ❌ whole-arg map-valued driver traversals; nested map KEYS |
| `addLabel()`, `dropLabel()`, `dropLabels()` | ✅ | a sideEffect over an existing vertex stream, passing the same vertices on: string names, a compile-time `constant(…)` string or list, and a collection as the SOLE argument. The cardinality FLOOR decides where each refusal lives — `dropLabels()` under `min > 0` and every immutable graph are compile-time declines, a named drop that would fall below `min` is a guard binding raising before the delete. ❌ an EDGE (label cardinality fixed at ONE by spec) and a per-row nested label |
| read tail after `property()`/`addLabel()` | ✅ | the element is preserved, so the suffix is an ordinary read over the mutated elements (`property(k,v).valueMap()`, `.out().values()`). ❌ a GLOBAL barrier (`count`/`fold`/`order`/a slice) over MORE THAN ONE element — the continuation reads once per element, so it fails closed rather than answering per element; ❌ a mutating tail |
