...
| Step | | Notes |
|---|:--:|---|
| `union(a, b…)` | 🟡 | two or more arms ✅, over an ordered input or with an arm-local `order()`/`limit()` (a union is UNORDERED, so the position is dropped). REDUCTION arms (each ends in a collapsing barrier — `union(__.count(), __.out().count())` → `[6,4]`, `union(__.min(),__.max())`) lower ARM-major: each reduces over the whole input, gated on the source being non-empty. A MIXED reduction+streaming set works too, including a cross-SHAPE mix as a variant (`union(__.min(), __.constant(99))` → `[27,99,99,99,99]`, `union(__.count(), __.out())` → `[1, v…]`). ❌ the SINGLE-arm form; a MIXED variant with a list-of-ELEMENTS arm (`union(__.fold(), __.out())` — the variant framer has no element expansion inside a list); an alias through a collapsed arm; a union whose emission order a downstream slice/collect READS (`union(…).limit(n)`, `.fold()`, `.cap()` — needs the arm-blocked fan-out order, not yet minted) |
