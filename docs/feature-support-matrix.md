...
no note, the whole step works. **L3 conformance: <!-- L3:passing -->1,474<!-- /L3:passing --> · corpus parse+chain: 2298/2298.**
...
| `repeat().path()`, `simplePath()` in body | ✅ | JSONB array walk + `json_each` cycle guard. `by(key)` and `by(T.id)`/`by(T.label)` go through the SAME position projector the linear regime uses (parameterized by how the element is reached — a joined table vs a correlated read off the exploded id), so a given `by()` answers identically in both regimes. ❌ `by(traversal)` over a recursive path (needs a positional child keyed to the exploded element; the linear regime has that substrate, this one does not yet) and multiple `by()`s (round-robin over a dynamic length) |
