...
(`count`, the `limit`/`range`/`skip` slices) compose over it. A payload-reading step (`unfold`, a member
transform, a value `dedup`) does not — a variant has no uniform member shape.
