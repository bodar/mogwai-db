...
no note, the whole step works. **L3 conformance: <!-- L3:passing -->1,273<!-- /L3:passing --> · corpus parse+chain: 2298/2298.**
...
| `sack()` / `withSack(…)` | 🟡 | carried column: `sack(Operator.x).by(key/T.label/nested)` mutate, bare `sack()` read, `withSack(init)` seed. **Folds through `repeat()`**: a body `sack(op).by(key/T.label/constant)` folds the accumulator per iteration (split-only per fork, TinkerPop-correct), and `where(__.sack().is(P))` guards the walk on the freshly-folded value. ❌ inject-const numeric promotion; `barrier`/`local`; branch split/merge-on-fork; a fan-out `by(traversal)` or edge-step movement (`outE().inV()`) in a repeat body; `sack(BiFunction)` |
