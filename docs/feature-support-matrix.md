...
no note, the whole step works. **L3 conformance: <!-- L3:passing -->1,465<!-- /L3:passing --> · corpus parse+chain: 2298/2298.**
...
| `choose(fn).option(k, body)…` | ✅ | an ARM MERGE selected by an N-way lookup on the choice (first match wins), routed through the same triage + merges every branch uses: ELEMENT bodies (`option('x', __.out('knows'))`), scalar, and mixed. **No `Pick.none`** is the TinkerPop pass-through — unmatched inputs emit the ELEMENT itself, so the result is a mixed scalar/element variant stream. A `__.discard()` body drops its rows (contributing no arm); `__.identity()` is an element arm. When every body is one scalar per input the merge collapses to a single correlated `CASE` (the fast path), incl. nested scalar option maps inside `map`/`local`/`flatMap`. ❌ LIST (`…fold()`) bodies inside a `local()`/`map()` child — the child classifier, not the merge; `Pick.unproductive`/`any`; a `T`-token choice over a scalar parent |
