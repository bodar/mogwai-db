...
**L3 conformance: <!-- L3:passing -->1,710<!-- /L3:passing -->/2,260 · corpus parse+chain: 2,395/2,395.**
...
| `mogwai.graph.federate` | ✅ | cross-graph pushdown (async barrier). Source form runs a rooted sub-traversal on a sibling → detached refs; mid-traversal injects each parent's `values`/`id`/`label` via the `T.value` marker, batched one hop and value-rejoined per parent. A detached result supports `id`/`label`/`values`. **`.with("subgraph", true)`** over an EDGE-producing sub-traversal brings back a traversable SUBGRAPH — the edges (with `src`/`tgt` adjacency) plus their incident vertices WITH data, landed as bound relations — walked locally like a `sg.traversal()` (movement over a bound edge `Ref`, `docs/2026-08-21-barrier-substrate-design.md` §B): `inV`/`outV`/`bothV` off the edge stream, **`.V()`/`.E()` re-source** at the subgraph, and **`out`/`in`/`both` (vertex→vertex, inline labels)** join the bound edges to the bound vertices — the reached element re-entering with full data so `values`/`id`/`label` and further hops compose. **`has(key)`/`has(key,value)`/`has(key,P)`** (the six comparison predicates) and **`hasLabel(l…)`** filter bound vertices (an `EXISTS` over the exploded `{t,v}` tree / label array — multi-valued membership), composing after `.V()` and after a movement hop. **`count()`/`dedup()`** compose over the landed stream (a detached result OR a subgraph) — `count` reads cardinality, `dedup` collapses by element id. `has(key, within(…))`, the 3-arg `has(label,key,value)`, and `V(ids)`/`E(ids)` (inline ids) select over the bound source. ❌ `has(key, without/textP/composed)`, bound-parameter/`null` movement labels, `limit`/`order` over the bound stream — next slices; `path()`/`as()` spanning the call; local movement over a NON-subgraph detached result |
...
| `split` | 🟡 | GLOBAL `split(sep)` over a SCALAR string stream is a value-transform **barrier** (`compiler/rel/split.ts`, substrate A — a JS map re-injected as a `json_each` LIST source, framed back as lists). Faithful to `SplitGlobalStep`/`StringUtil.split`: an empty separator → characters, a null separator → whitespace, else a whole-separator split with empty tokens removed; null passes through; a non-string traverser RAISES the reference's `IllegalArgumentException`. The produced lists re-enter the ordinary list vocabulary (`split(sep).unfold()`). ❌ `split(Scope.local, sep)` over a folded list (a member-wise string→LIST transform), and a LIST-shaped head — both fail-closed deferrals |
