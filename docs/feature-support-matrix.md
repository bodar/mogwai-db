...
| `coalesce(…)` | 🟡 | first-productive over element/scalar/list arms (empty `fold()` is productive); element movement + `limit`/`skip`/`range`/`dedup`; nests in coalesce/optional. **Over a scalar parent**: first arm that produces a value per row (productivity = the scalar-arm predicate); value/reducer/nested-branch/`V()`-`E()`-re-source arm bodies. ❌ scalar-parent mixed-shape arms → variant (follow-on) |
...
| `map(__.…)` | 🟡 | scope-aware child barriers (`count/sum/min/max/mean`), scalar tails, `fold()` per parent, movement bodies (first-per-origin). **Over a scalar parent**: value/reducer/nested-branch arm body per value (a filtering body drops non-productive inputs), plus a `V()`/`E()` re-source that reduces/projects back to a scalar per input. ❌ alias/select/structured bodies; a bare re-source ending in element space (→ variant, follow-on) |
