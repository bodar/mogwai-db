...
no note, the whole step works. **L3 conformance: <!-- L3:passing -->1,277<!-- /L3:passing --> · corpus parse+chain: 2298/2298.**
...
| `union(…)` | ✅ | element multi-hop/nested arms; scalar/list arms; over a scalar parent; every arm shape's merge mints the canonical arm-ordered `encounter` (arm 0 fully before arm 1) when a positional consumer demands emission order. ❌ full-`V()`/`E()` source-branch tails; `as()` inside a non-element arm (defers — the alias would not survive the merge, see `outstanding-work.md` item 1's residual) |
