...
**L3 conformance: <!-- L3:passing -->1,431<!-- /L3:passing -->/2,260 · corpus parse+chain: 2,395/2,395.**
...
| `choose(pred, a, b)`, `choose(…).option(…)` | ✅ | |
| `coalesce`, `optional`, `branch`, `map(__.…)`, `flatMap`, `sideEffect(__.…)` | ❌ | |
| `local(__.…)` | ❌ | |
