...
no note, the whole step works. **L3 conformance: <!-- L3:passing -->1,759<!-- /L3:passing --> · corpus parse+chain: 2298/2298.**
...
| `mergeV`, `mergeE` | 🟡 | id-aware upsert, onCreate/onMatch, start + mid-chain, and over a SCALAR stream (`g.inject(0).mergeV(…)`) — a creation's input is a MULTIPLIER, so its traverser need not be an element; map label/id/VALUES may be nested traversals; prop VALUES keep their type. ❌ whole-arg map-valued driver traversals; nested map KEYS; an ENDPOINT-LESS `mergeE` (no `Direction` key — the search is `g.E()` and a create is impossible) |
