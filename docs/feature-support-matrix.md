...
| `flatMap(__.…)` | 🟡 | ✅ origin-safe movement/filter element bodies and scalar `values`/`id`/`label`/`constant` tails use the generic child compiler with `all` cardinality; transforms + row operators + scoped `count/sum/min/max/mean` share stream/barrier lowering with explicit encounter order<br>✅ **incoming `as()` threads through** (single body, no merge)<br>✅ **path threads through** element bodies (pad-to-max cols)<br>❌ record/list/group/path/fold bodies; NEW `as()` inside body |
| `map(__.…)` | 🟡 | ✅ child `count()` and scalar-tail `count/sum/min/max/mean` are scope-aware barriers: parent-domain LEFT JOIN + origin GROUP BY preserves duplicates/empty children; numeric `v,vt` survives later scalar composition<br>✅ scalar `values`/`id`/`label`/`constant` tails are productive rows through the generic child domain; missing values drop the parent, productive NULL survives; transforms and `is/order/limit/skip/range/dedup` run before first-per-origin through the shared scalar pipeline<br>✅ movement/filter element bodies compile through the generic child domain; explicit encounter metadata + `ROW_NUMBER() PARTITION BY origin` keeps the first productive child per multiset-distinct parent<br>❌ alias/select/fold and other structured barrier-bearing bodies |
| `local(…)` | 🟡 | ✅ scalar movement+projection bodies use generic child `all` cardinality; transforms, `is/order/limit/skip/range/dedup`, and `count/sum/min/max/mean` are origin-partitioned through shared lowering<br>✅ element movement + per-element `limit()`/`range()` retains the legacy child-domain window pending generic element barriers<br>✅ outer `as()` aliases/path columns survive the child scope<br>❌ structured properties/record/list/group/path/fold bodies; match/simplePath/union/nested local; `local(aggregate(...))`; sack/otherV state |
