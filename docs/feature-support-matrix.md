...
| `mogwai.graph.federate` | ✅ | cross-graph pushdown (async barrier). Source form runs a rooted sub-traversal on a sibling → detached refs; mid-traversal injects each parent's `values`/`id`/`label` via the `T.value` marker, batched one hop and value-rejoined per parent. A detached result supports `id`/`label`/`values`. **`.with("subgraph", true)`** over an EDGE-producing sub-traversal brings back a traversable SUBGRAPH — the edges (with `src`/`tgt` adjacency) plus their incident vertices WITH data, landed as bound relations — and `inV`/`outV`/`bothV` walk it locally (movement over a bound edge `Ref`, `docs/2026-08-21-barrier-substrate-design.md` §B), the endpoint re-entering with full data so `values`/`id`/`label` compose. ❌ `out`/`in`/`both` (vertex→vertex) over a subgraph, and `has()`/filters over subgraph vertices — next slice; `path()`/`as()` spanning the call; local movement over a NON-subgraph detached result |
