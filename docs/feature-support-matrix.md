...
| `select('a')`, multi-`select`, `project(…)` | 🟡 | ✅ column-threaded aliases<br>✅ single-label `select` retypes to an element (vertex/edge) or ScalarStream under `by(key)`; later steps compose<br>✅ multi-`select`/`project` lower to a heterogeneous per-traverser RecordStream (scalar/vertex/**edge** fields); selecting a field re-enters ordinary scalar/element lowering, and `count()` composes<br>✅ mixed `project(...).by(__.<scalar child>|key|T.id|T.label|bare element)`: fields use generic child-first productivity joined by a multiset-safe outer origin; productive NULL survives; vertex/edge payloads retain movement re-entry<br>✅ single/multi-label `select(...).by(__.<scalar child>)`: each child re-roots on its alias; direct/bare fields mix through the same origin join<br>✅ `limit`/`range`/`skip`/`tail` with `Scope.local` slice record fields<br>❌ list/element-valued traversal select; record `order`/`dedup`/`fold`/`where` |
