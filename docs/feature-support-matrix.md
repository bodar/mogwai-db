...
| Step | | Notes |
|---|:--:|---|
| `union(a, b…)` | 🟡 | two or more arms ✅, over an ordered input or with an arm-local `order()`/`limit()` (a union is UNORDERED, so the position is dropped). ALL-BATCHED arms (every arm holds a barrier — `union(__.count(), __.out().count())` → `[6,4]`, `union(__.min(),__.max())`) lower ARM-major: each arm reduces over the whole input, gated on the source being non-empty. ❌ the SINGLE-arm form; a MIXED batched/streaming arm set (`union(__.min(), __.constant(99))`); an alias through a collapsed arm; a union whose emission order a downstream slice/collect READS (`union(…).limit(n)`, `.fold()`, `.cap()` — needs the arm-blocked fan-out order, not yet minted) |
