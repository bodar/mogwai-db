...
| `flatMap(__.…)` | 🟡 | ✅ origin-safe movement/filter element bodies and scalar `values`/`id`/`label`/`constant` tails use the generic child compiler with `all` cardinality; transforms plus `is/order/limit/skip/range/dedup` reuse origin-partitioned `lowerScalarRows` with explicit encounter order<br>✅ **incoming `as()` threads through** (single body, no merge)<br>✅ **path threads through** element bodies (pad-to-max cols)<br>❌ record/list/group/path bodies; origin-scoped scalar reducers/fold; NEW `as()` inside body |
