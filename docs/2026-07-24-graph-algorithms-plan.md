...
Design/research doc — scoping + architecture, nothing built. Names the open research where it matters.
...
Builds on `docs/archive/2026-07-20-call-service-registry-plan.md` (the `call()` + Service Registry
seam) — read that first. Nothing here changes the compiler core; it adds services and rewrite Passes.
...
6. **Rides the existing seam — no new orchestrator.** Global iterative algorithms are `'barrier'`
   contributions (the async/multi-statement variant already in the registry). Results flow back through
   the ordinary `lowerSteps`/`materializeRootStream` machinery and the existing write path for `mutate`.
...
- **Named step → `call()` is an `extract` Pass**, not a StepFn. Idiomatic: the pipeline already does
  `Step[]→Step[]` rewrites, and `extract` runs first so the rest of the pipeline (and the `verify` pass,
  which asserts against `ctx.originalChain`) sees the canonical form while error messages still
  reference what the user wrote. **Do NOT add a `pageRank` StepFn** — a second implementation, violates
  the extension law + principle #2.
- **The service is a `'barrier'` contribution** (async/multi-statement) — the variant already reserved
  in the registry for exactly this. `g.call(...)` = `seedCall` (start form); `V().call(...)` =
  `lowerCall` (per-parent child scope, the element-preserving form).
...
`tinker.degree.centrality` uses (`type: 'streaming'`, per-parent merge, element carried,
`scopedMovementCount` → `pushChildScope` + `lowerScopedScalarReducer`). The *computation* is global
(barrier), but the *binding* is a per-traverser join to the `id→score` table. Get this split right and
the OLAP-excluded conformance scenarios ("PageRank=3 is OLAP-excluded") come into scope as a give-back.
...
- `mutate` — write score back as a vertex property via the existing write path (`applyVertexProperty`);
  then it's a normal queryable property. The ergonomic escape hatch; moots the return-shape question.
...
**Template B (shortestPath):** single recursive CTE producing paths; different tail (`PathStream`); no
barrier needed for unweighted.
...
segmented-plan model in the call-registry doc defines):
...
- The traverser-`bulk` column and `movementCollapse` substrate are the model for the frontier arithmetic.

### 3. Binding back to traversers (the three tails)
- **decorate mode** (native step): the incoming `ElementStream` JOINs `pr` on `id`, the score rides as
  a carried scalar exposed under `resultKey`; the stream stays elements → `order()/by()/values()`
...
- **mutate mode**: write `pr` into `vertex_properties` via `applyVertexProperty` (single cardinality),
  keyed by `resultKey`. Then normal queries read it.
...
1. **Barrier/segmented-plan wiring** (unlocks EVERY iterative algorithm) — how a `'barrier'`
   contribution actually suspends between SQL segments today (the call-registry doc describes the model;
   confirm what its "as built" addendum built) and whether a `decorate`-mode global compute can complete
   in-request.
2. **SQLite recursive-CTE limits** for the Tier-1 path family (no aggregate/`GROUP BY` in the recursive
   term) — drives which algos must be host-driven vs single-CTE, i.e. the whole path family's shape.
...
4. **DO alarm checkpointing** for large-graph iterative jobs (`stream`/`mutate`/`stats` only).
5. **PageRank dangling-node handling** the conformance scenarios expect (leaf — PageRank only).
