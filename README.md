...
- src/server.ts    — HTTP endpoint + GraphBinary response framing (reuses the client
                     package's Apache-2.0 serializers)
- test/e2e.ts      — end-to-end suite via the real GLV
- parser/          — generated from gremlin-language/Gremlin.g4 (regenerate, don't edit)

## Run
npm install
npx tsx src/server.ts          # listens on :8182
npx tsx test/e2e.ts            # 11 checks, all passing
...
- Port storage.ts to DO SQLite (ctx.storage.sql) + Worker fetch handler; better-sqlite3
  is a shim with matching semantics
