...
// compatible with CouchDB — §9·1 — so the exact algorithm is free, only its DETERMINISM matters). It
// is the standard FIPS-180-4 SHA-256, vector-tested in test/hash.test.ts.
//
// The output is used for content-addressing (idempotent replay: identical content → identical hash)
// and the deterministic conflict-winner tiebreak (§6·3), neither of which needs cryptographic strength
// — only a stable, low-collision function every mogwai instance computes identically.
...
/** SHA-256 of a UTF-8 string → 64-char lowercase hex. Standard FIPS-180-4. */
export function sha256Hex(input: string): string {
  ...
  // bit length as two 32-bit words (length < 2^53, so the high word is floor(bitLen / 2^32)).
