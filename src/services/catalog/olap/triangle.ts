...
const TRI = q`tri(v, c) AS (SELECT nv.x, COUNT(*) FROM und nv
  JOIN und nw ON nv.x = nw.x AND nv.y < nw.y
  JOIN und e ON e.x = nv.y AND e.y = nw.y GROUP BY nv.x)`;

/** A one-shot decorate barrier: `apply` runs ONE INSERT..SELECT (`sql`, built for the run token) that
 *  writes the score per vertex at (round 0, scope 0, channel 0); the decorate resume reads it. */
function oneShotDecorate(serviceName: string, defaultKey: string, vtype: string, sql: (run: number) => Expression, store: GraphStore | undefined): Service {
  return decorateBarrier({
    ...
    plan: (params) => ({
      ...
      core: (store, run): number => { execute(store, sql(run)); return 0; },
    }),
  });
}
...
export const createTriangleCountService = (store: GraphStore | undefined): Service =>
  oneShotDecorate(TRIANGLE_COUNT_SERVICE_NAME, 'triangleCount', 'int',
    (run) => q`WITH ${UND}, ${TRI}
      ${STATE_INSERT} SELECT ${value(run)}, 0, 0, n.id, 0, COALESCE(tri.c, 0) FROM nodes n LEFT JOIN tri ON tri.v = n.id`,
    store);
...
export const createLocalClusteringService = (store: GraphStore | undefined): Service =>
  oneShotDecorate(LCC_SERVICE_NAME, 'localClusteringCoefficient', 'double',
    (run) => q`WITH ${UND}, ${TRI}, deg(id, d) AS (SELECT x, COUNT(*) FROM und GROUP BY x)
      ${STATE_INSERT} SELECT ${value(run)}, 0, 0, n.id, 0,
