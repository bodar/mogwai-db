...
import { createPageRankService, createPeerPressureService, createWccService, pendingGraphAlgorithmServices } from './catalog/graph-algorithms.ts';
...
export const standardRegistry: RegistryProvider = (app) =>
  createRegistry([createDirectoryService(app), degreeCentralityService, searchService, createIoService(app.io, app.store),
    ...pendingGraphAlgorithmServices, createWccService(app.store), createPageRankService(app.store), createPeerPressureService(app.store)]);
...
export const extendedRegistry: RegistryProvider = (app) =>
  createRegistry([createDirectoryService(app), degreeCentralityService, searchService, createIoService(app.io, app.store),
    createFederateService(app.source), schemaService, ...pendingGraphAlgorithmServices, createWccService(app.store), createPageRankService(app.store), createPeerPressureService(app.store)]);
