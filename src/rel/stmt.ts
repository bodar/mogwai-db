...
export interface Insert { readonly kind: 'insert'; readonly target: StmtTarget; readonly cols: readonly string[]; readonly source: Rel; readonly onConflict?: { readonly target: readonly string[]; readonly set: readonly (readonly [string, Expr])[] }; readonly returning: readonly (readonly [string, Expr])[]; }
export interface Update { readonly kind: 'update'; readonly target: StmtTarget; readonly set: readonly (readonly [string, Expr])[]; readonly from?: Rel; readonly where?: Expr; readonly returning: readonly (readonly [string, Expr])[]; }
export interface Delete { readonly kind: 'delete'; readonly target: StmtTarget; readonly where?: Expr; readonly using?: Rel; readonly returning: readonly (readonly [string, Expr])[]; }
