...
import { withCarried, type Carry, type ElementStream, type StepFn } from '../../steps/context.ts';
import { move, toEdge, toVertex, otherV } from '../../steps/movement.ts';
import { as, hasLabel, has, hasId, where, andOr, dedup, simplePath, cyclicPath } from '../../steps/filter.ts';
import { union, optional, repeat, choose, coalesce } from '../../steps/branch.ts';
import { isElementChild, isListChild, isScalarChild } from '../../steps/child.ts';
import { match } from '../../steps/match.ts';
import { identity, limit, range, skip } from '../../steps/passthrough.ts';
import { sack } from '../../steps/sack.ts';
import { aggregate, group as groupSE, groupCount as groupCountSE } from '../../steps/sideeffect.ts';
import { type SackSpec } from '../../gremlin/frontend.ts';
import { compileTail, compileFromScalar } from '../../steps/projection.ts';
import { compileFromGroup, compileFromProperty } from '../../steps/group.ts';
import { compileFromList, compileFromMap, compileFromMapEntry } from '../../steps/list.ts';
import { compileFromRecord, compileFromPath, selectRecordFromAlias } from '../../steps/select.ts';
import { asOnStream, selectOneFromAlias } from '../../steps/labelselect.ts';
import { assertStreamColumns, continueLowering, isSuspension, type LoweringResult, type LoweringSuspension, type Stream } from '../../steps/stream.ts';
import { type Compiled } from '../../sql/kernel/render.ts';
import { tryBulkRepeat } from '../../steps/bulk.ts';
import { type FastPathConfig } from '../options/fast-paths.ts';
import type { ServiceRegistry } from '../../services/types.ts';
import { lowerScalarRows } from '../../steps/scalar.ts';
import { seedCall, isBarrierPoint, type BarrierPoint, type MidBarrierPoint } from '../../steps/call.ts';
import { materializeFinal } from '../../steps/materialize.ts';
import { compileFromVariant } from '../../steps/variant.ts';
import { compileFromForeign, landForeignElements, resumeMidBarrier } from '../../steps/foreign.ts';
