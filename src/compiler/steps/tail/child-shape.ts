...
import { isColumnArg, isOperatorArg, isOrderArg, isPickArg, isTokenArg, isNested, stepChain, argValues } from '../../../gremlin/frontend.ts';
...
import { assertsGType, asBranchKind, collectionAssert, isStreamBarrier, REDUCERS, SCALAR_TRANSFORMS, typeOfAssert, type BranchKind, type TypeAssert } from '../../ir/step.ts';
...
/** Child chains cross the same normalization seam as the root — re-exported from `ir/passes.ts`,
 *  beside the `normalize` it is built on. */
export { childSteps };
...
/** The `is(typeOf(GType.X))` decode and its two derived readings — re-exported from `ir/step.ts`,
 *  beside `sliceOf`, the other total decode of one step's arguments. */
export { assertsGType, collectionAssert, typeOfAssert, type TypeAssert };
