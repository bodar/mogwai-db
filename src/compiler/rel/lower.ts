...
import { armBatches, assertsGType, collectionAssert, isLocalScope, isStreamBarrier, PATH_LIST_OPS, type Slice, typeOfAssert } from '../ir/step.ts';
...
import { propertyElement, propertyHasClause, propertyId, propertyKey, propertyPayload, propertyReadOf, propertyRowId, propertyValue } from './property.ts';
...
import {
    ...
    jsonMemberByTypeof, labelSetArgs, meta, minter, notProduced,
...
import { appendPathLabel, appendValuePosition, PATH_CHANNEL, pathCarried, pathPayload, pathPositions, pathSimpleByPredicate, seedPath, subPathMembers } from './path.ts';
...
import { BULK, CARRIED_READ, ENCOUNTER, NO_ALIASES, ORIGIN, bodyOf, encounterOf, inBody, originOf, type ChainCtx, type ElementSubject, type Tail } from './lower/chain.ts';
import { HOPS, movement, reSource } from './lower/movement.ts';
import { dedupByLabels, dedupOn, elementRowShape, propertyRowShape, rowOp, sliceOp, PER_TRAVERSER_HOSTS, ROW_OPS } from './lower/slice.ts';
import { branchSubject, bodyPredicate, childHostOf, childPredicate, correlatedExists, sourceFilter, valuePredicate } from './lower/filter.ts';
