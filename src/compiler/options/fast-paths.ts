...
export interface CompileOptions {
  ...
  /** Pin this compile's spine POSITION, overriding the ambient `MOGWAI_RELIR` switch
   *  (options/spine.ts). `'legacy'` forces the pre-RelIR lowering even for a covered traversal;
   *  `'rel'` asks for the RelIR route, which still declines to legacy for a chain it does not cover
   *  — coverage is a property of the chain, never of the request; `'rel-only'` asks for RelIR and
   *  RAISES rather than falling back, which is the measurement position, not a production one. */
  readonly spine?: import('./spine.ts').SpinePosition;
}
