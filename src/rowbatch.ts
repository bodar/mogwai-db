...
//
// The chunked form is also the FAST one, which is what makes this a free invariant rather than a
// portability tax: measured 4.6× faster than inlining literals to dodge the cap (38 ms vs 176 ms for
// 20,000 rows), because a fixed-shape statement hits the prepared-statement cache (`bun:sqlite`
// caches by SQL text, and so does DO) while a big inlined statement is a fresh parse that evicts it.
//
