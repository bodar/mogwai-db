...
`~/Projects/the client` — see `src/Application.ts` (the agnostic `application(deps)`
builder), `src/cloudflare/app.ts` and `src/bun/app.ts` (the two entry points that
provide platform `deps`), and `src/database/TheClientDatabase.ts` +
...
- **Stays synchronous — deliberate divergence from the client.** TheClient's
  `TheClientDatabase` is async because D1 is async. We target DO
