// Drives the graph-worker TRANSPORT: this driver worker spawns the dedicated graph-worker entry as a
// nested Worker, wraps it in GraphWorkerClient, and exercises the postMessage RPC — open, seed, read
...
async function read(client: GraphWorkerClient, gremlin: string): Promise<any[]> {
  ...
  const framed: Framed[] = (await client.framed(gremlin, {})).map((f) => ({ buf: Buffer.from(f.buf), bulk: f.bulk }));
...
self.onmessage = async () => {
  try {
    ...
    const opened = await client.open(`gwrpc-${Date.now()}-${Math.floor(Math.random() * 1e6)}`);
    ...
    await client.framed("g.addV('person').property('name','marko').property('age',29)", {});
    await client.framed("g.addV('person').property('name','vadas').property('age',27)", {});
    ...
    await check('a failing query rejects as a value (does not hang the RPC)', async () => {
      let threw = false;
      try { await client.framed('g.V().sack()', {}); } catch { threw = true; }
