...
describe('scalar-parent branch/map (Stage 1)', () => {
  ...
  test('choose(P, then, else) over a scalar → gated UNION ALL (no inline CASE)', async () => {
    ...
    expect(await vals("g.V().hasLabel('person').values('age').choose(eq(29),__.constant('m'),__.constant('o'))"))
      .toEqual(['m', 'o', 'o', 'o']);
  });

  test('choose(P, then) with no else → identity passthrough of the value', async () => {
    expect(await vals("g.V().hasLabel('person').values('age').choose(eq(29),__.constant('m'))"))
      .toEqual(['27', '32', '35', 'm'].sort());
  });

  test('choose(traversal-predicate, then, else) over a scalar', async () => {
    expect(await vals("g.V().hasLabel('person').values('age').choose(__.is(gt(30)),__.constant('big'),__.constant('small'))"))
      .toEqual(['big', 'big', 'small', 'small']);
  });

  test('map()/local()/flatMap() over a scalar apply the value body', async () => {
    expect(await vals("g.V().hasLabel('person').values('age').map(__.constant('x'))")).toEqual(['x', 'x', 'x', 'x']);
    expect(await vals("g.V().hasLabel('person').values('name').map(__.toUpper())")).toEqual(['JOSH', 'MARKO', 'PETER', 'VADAS']);
    expect(await vals("g.V().hasLabel('person').values('name').local(__.toUpper())")).toEqual(['JOSH', 'MARKO', 'PETER', 'VADAS']);
  });

  test('map() with a filtering body drops non-productive inputs', async () => {
  ...
  test('coalesce() over a scalar → first arm that produces, per row', async () => {
    expect(await vals("g.V().hasLabel('person').values('age').coalesce(__.is(gt(30)),__.constant('lo'))"))
  ...
  test('tail()/dedup() arms lower per input via the child scope', async () => {
    expect(await vals("g.V().hasLabel('person').values('age').union(__.tail(1),__.identity())"))
      .toEqual(['27', '27', '29', '29', '32', '32', '35', '35']);
    expect(await vals("g.V().hasLabel('person').as('a').values('age').union(__.dedup(),__.identity())"))
      .toEqual(['27', '27', '29', '29', '32', '32', '35', '35']);
  });
  ...
  test('V()/E() re-source arms reduce per input', async () => {
    // bare re-source count: all 6 vertices per input age → 6 each.
    expect(await vals("g.V().hasLabel('person').values('age').map(__.V().count())")).toEqual(['6', '6', '6', '6']);
    // movement then count: out() across all 6 vertices = 6 edges → 6 per input.
    expect(await vals("g.V().hasLabel('person').values('age').map(__.V().out().count())")).toEqual(['6', '6', '6', '6']);
    // re-source + projection (multi-value per input), one input via V(1).
    expect(await vals("g.V(1).values('age').flatMap(__.V().hasLabel('person').values('name'))"))
      .toEqual(['josh', 'marko', 'peter', 'vadas']);
    // re-source + numeric reducer over the projected values.
    expect(await vals("g.V(1).values('age').map(__.V().hasLabel('person').values('age').sum())")).toEqual(['123']);
  });
  ...
  test('map() is first-result-only: fan-out arm bodies take the first emitted result', async () => {
    // fan-out via a nested union: flatMap/local → all 8; map → the first arm's value per input.
    expect(await vals("g.V().hasLabel('person').values('age').flatMap(__.union(__.constant('lo'),__.constant('hi')))"))
      .toEqual(['hi', 'hi', 'hi', 'hi', 'lo', 'lo', 'lo', 'lo']);
    expect(await vals("g.V().hasLabel('person').values('age').local(__.union(__.constant('lo'),__.constant('hi')))"))
      .toEqual(['hi', 'hi', 'hi', 'hi', 'lo', 'lo', 'lo', 'lo']);
    // map → arm 0 ('lo') per input (4 person ages).
    expect(await vals("g.V().hasLabel('person').values('age').map(__.union(__.constant('lo'),__.constant('hi')))"))
      .toEqual(['lo', 'lo', 'lo', 'lo']);
    // fan-out via a re-source projection: flatMap → all names; map → the first (element-id order).
    expect(await vals("g.V(1).values('age').flatMap(__.V().hasLabel('person').values('name'))"))
      .toEqual(['josh', 'marko', 'peter', 'vadas']);
    expect(await vals("g.V(1).values('age').map(__.V().values('name'))")).toEqual(['marko']);
    // ≤1 bodies stay identical under map (transform, choose 1:1, re-source+reducer).
    expect(await vals("g.V().hasLabel('person').values('name').map(__.toUpper())")).toEqual(['JOSH', 'MARKO', 'PETER', 'VADAS']);
    expect(await vals("g.V().hasLabel('person').values('age').map(__.V().count())")).toEqual(['6', '6', '6', '6']);
  });

  test('re-source arms compose in union/choose/coalesce', async () => {
    expect(await vals("g.V(1).values('age').union(__.constant('x'),__.V().count())")).toEqual(['6', 'x']);
    // choose: age 29 is not > 30 → the else (constant 0) wins.
    expect(await vals("g.V(1).values('age').choose(__.is(gt(30)),__.V().count(),__.constant(0))")).toEqual(['0']);
    // coalesce: the first (impossible) predicate never fires → every input falls to V().count().
    expect(await vals("g.V().hasLabel('person').values('age').coalesce(__.is(gt(100)),__.V().count())"))
      .toEqual(['6', '6', '6', '6']);
  });
...
describe('scalar math (Stage 2)', () => {
  ...
  test('named variables resolve through by()-modulators over the value', async () => {
    ...
    const v = async (g: string) =>
      (await decodeAll(executeQuery(store2, g, {}))).map(String).sort();
    expect(await v("g.V().values('age').math('_ + a').by(__.constant(100))")).toEqual(['127', '129']);
    expect(await v("g.V().values('age').math('a * b').by(__.identity()).by(__.constant(2))")).toEqual(['54', '58']);
  });
...
describe('V()/E() after a scalar — mid-traversal re-source (Stage 4)', () => {
  ...
  test('inject(x).V() produces all vertices per traverser (multiset), V(id) the id-matched', async () => {
    expect(await decodeAll(executeQuery(store, 'g.inject(0).V().count()', {}))).toEqual([2]);
    expect(await decodeAll(executeQuery(store, 'g.inject(1,2).V().count()', {}))).toEqual([4]); // 2 traversers × 2
    expect(await names("g.inject(0).V(1).values('name')")).toEqual(['marko']);
  });

  test('a value-alias survives the re-source and reads as its value', async () => {
    expect(await decodeAll(executeQuery(store, "g.inject(1).as('age').V().select('age')", {}))).toEqual([1, 1]);
  });

  test("format()'s named token falls back to an as()-label when the property is absent", async () => {
    expect(await names("g.inject(1).as('age').V().format('%{name} is %{age} years old')"))
      .toEqual(['lop is 1 years old', 'marko is 29 years old']); // software has no age → alias 1
    expect(await names("g.V().format('%{name} is %{age} years old')")).toEqual(['marko is 29 years old']); // no alias → lop filtered
  });
});
...
describe('scalar split (Stage 2)', () => {
  const dec = async (b: Buffer) => { const x = await decode(b); return x === null ? null : (x as any[]).map((y: any) => y?.v ?? y); };
  const lists = async (g: string) => {
    const store = new GraphStore(new BunSqlite(':memory:'));
    const out: any[] = [];
    for (const b of executeQuery(store, g, {})) out.push(await dec(b));
    return out;
  };
  test('split(sep) splits on each occurrence', async () => {
    expect(await lists('g.inject("marko","vadas","josh").split("a")')).toEqual([['m', 'rko'], ['v', 'd', 's'], ['josh']]);
  });
  test('split("") splits into characters; a null value stays null', async () => {
    expect(await lists('g.inject("that","this","test",null).split("")')).toEqual([['t', 'h', 'a', 't'], ['t', 'h', 'i', 's'], ['t', 'e', 's', 't'], null]);
  });
  test('split(null) splits on whitespace runs', async () => {
    expect(await lists('g.inject("hello world","marko").split(null)')).toEqual([['hello', 'world'], ['marko']]);
  });
...
describe('scalar tail at root (Stage 2 fix)', () => {
  ...
  test('tail(N) takes the last N; bare tail() the last one', async () => {
    expect(await vals('g.inject(1,2,3,4).tail(2)')).toEqual(['3', '4']);
    expect(await vals('g.inject(1,2,3,4).tail()')).toEqual(['4']);
    expect(await vals('g.inject(3,1,2).order().tail(1)')).toEqual(['3']);
  });
...
describe('scalar child scope — pushChildScope substrate (Stage 2)', () => {
  ...
  test('a reducer body reduces the single value per traverser', async () => {
    expect(await vals("g.V().hasLabel('p').values('age').map(__.count())")).toEqual(['1', '1']);   // count of one value
    expect(await vals("g.V().hasLabel('p').values('age').local(__.sum())")).toEqual(['27', '29']); // sum of one value
    expect(await vals("g.V().hasLabel('p').values('age').map(__.max())")).toEqual(['27', '29']);
    expect(await vals("g.V().hasLabel('p').values('age').map(__.mean())")).toEqual(['27', '29']);
  });

  test('a value-op prefix composes before the scoped reducer', async () => {
    expect(await vals("g.V().hasLabel('p').values('name').map(__.toUpper().count())")).toEqual(['1', '1']);
  });

  test('cardinality-preserving value bodies still take the light path', async () => {
    expect(await vals("g.V().hasLabel('p').values('age').map(__.constant('x'))")).toEqual(['x', 'x']);
  });
...
describe('scalar project — modulation over the value (Stage 2)', () => {
  ...
  test('a reducer field lowers through the scalar child scope', async () => {
    expect(await recs("g.V().hasLabel('p').values('age').project('v','c').by(__.identity()).by(__.count())"))
...
describe('scalar option-map choose (Stage 2)', () => {
  ...
  test('a predicate-keyed option-map over the value', async () => {
    expect(await vals("g.V().values('age').choose(__.identity()).option(between(26,30),__.constant('young')).option(Pick.none,__.constant('old'))"))
      .toEqual(['old', 'young', 'young']);
  });

  test('a literal-keyed option-map over the value', async () => {
    expect(await vals("g.V().values('age').choose(__.identity()).option(29,__.constant('marko')).option(Pick.none,__.constant('other'))"))
