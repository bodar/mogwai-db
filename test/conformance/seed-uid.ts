...
export function seedUid(store: GraphStore) {
  ...
  const node = 'INSERT INTO nodes(id, uid, label, props) VALUES(?,?,?,?)';
  store.query(node, [1, 'alice', person, JSON.stringify({ name: 'alice' })]);
  store.query(node, [2, 'bob', person, JSON.stringify({ name: 'bob' })]);

  const edge = 'INSERT INTO edges(id, uid, src, label, tgt, props) VALUES(?,?,?,?,?,?)';
