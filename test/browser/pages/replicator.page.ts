...
async function main() {
  // `installMogwai()` resolves once the Service Worker CONTROLS this page (registerServiceWorker), so the
  // first intercepted fetch below is not raced by activation. The registry Worker still spawns lazily on
  // first touch, but that is awaited event-based inside the edge (FactoryStubSource.openRegistry solicits a
  // port and waits for it), so no warm-up round-trip is needed — the checks run straight against the edge.
