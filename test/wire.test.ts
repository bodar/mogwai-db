...
test('parseRequest captures nested bound-map value types deeply (typed client)', async () => {
...
test('parseRequest reads the bulkResults request option (GraphBinary + JSON)', async () => {
  ...
  expect((await parseRequest(build([[fqStr('bulkResults'), fqBool(true)]]))).bulked).toBe(true);
  // bulkResults=false and absent both → false (flat frame, backwards-compatible default).
  expect((await parseRequest(build([[fqStr('bulkResults'), fqBool(false)]]))).bulked).toBe(false);
  expect((await parseRequest(build([[fqStr('g'), fqStr('gmodern')]]))).bulked).toBe(false);
  // JSON request path honours it too.
  expect((await parseRequest(Buffer.from(JSON.stringify({ gremlin: 'g.V()', bulkResults: true })))).bulked).toBe(true);
  expect((await parseRequest(Buffer.from(JSON.stringify({ gremlin: 'g.V()' })))).bulked).toBe(false);
});
