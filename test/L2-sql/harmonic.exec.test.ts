...
// Harmonic centrality — harmonic, closeness's sibling on the SAME scope-keyed distance relaxation
...
describe('harmonic — harmonic centrality (shares closeness distance substrate)', () => {
  test('matches GDS (a=0.375, b=0.5, c=0.375, d=0.25, e=0.25)', async () => {
    const s = Object.fromEntries(((await run(seeded(HARMONIC_SEED),
      `g.V().call("harmonic").project("name","harmonic").by("name").by("harmonic")`)).map(unmap) as any[])
  ...
  test('every vertex decorated; b (path centre) is the most harmonic-central', async () => {
    ...
    expect(await run(store, `g.V().call("harmonic").has("harmonic").count()`)).toEqual([5]);
    expect(await run(store, `g.V().call("harmonic").order().by("harmonic", Order.desc).limit(1).values("name")`))
      .toEqual(['b']);
  });
});
