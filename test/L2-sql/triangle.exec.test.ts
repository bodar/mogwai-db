...
// Triangle count + local clustering coefficient — mogwai.triangleCount / mogwai.localClusteringCoefficient,
...
describe('mogwai.triangleCount / mogwai.localClusteringCoefficient', () => {
  test('5-clique: every vertex is in 6 triangles, coefficient 1.0 (GDS clique5)', async () => {
    ...
    const tri = await by(seeded(seed), 'mogwai.triangleCount', 'triangleCount');
    expect(Object.values(tri)).toEqual([6, 6, 6, 6, 6]);
    const lcc = await by(seeded(seed), 'mogwai.localClusteringCoefficient', 'localClusteringCoefficient');
  ...
  test('a line a-b-c has no triangles → all 0, coefficient 0', async () => {
    ...
    expect(Object.values(await by(seeded(seed), 'mogwai.triangleCount', 'triangleCount'))).toEqual([0, 0, 0]);
    expect(Object.values(await by(seeded(seed), 'mogwai.localClusteringCoefficient', 'localClusteringCoefficient'))).toEqual([0, 0, 0]);
  });
  ...
  test('triangle {a,b,c} + pendant a-d separates count from coefficient', async () => {
    ...
    expect(await by(seeded(seed), 'mogwai.triangleCount', 'triangleCount')).toEqual({ a: 1, b: 1, c: 1, d: 0 });
    const lcc = await by(seeded(seed), 'mogwai.localClusteringCoefficient', 'localClusteringCoefficient');
  ...
  test('every vertex decorated; order().by(triangleCount) composes', async () => {
    ...
    expect(await run(store, `g.V().call("mogwai.triangleCount").has("triangleCount").count()`)).toEqual([4]);
    // a/b/c all have 1 triangle, so tiebreak by name for a determinate top (else the window is arbitrary).
    expect(await run(store, `g.V().call("mogwai.triangleCount").order().by("triangleCount", Order.desc).by("name").limit(1).values("name")`))
      .toEqual(['a']);
  });
});
