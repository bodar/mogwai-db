...
// k-core decomposition — mogwai.kcore, a BSP fixpoint decorate barrier. Coreness via the Montresor
...
describe('mogwai.kcore — coreness (h-index fixpoint)', () => {
  test('GDS graph: z=0, a=1, b=1, c..h=2', async () => {
    ...
    const core = Object.fromEntries(((await run(seeded(seed),
      `g.V().call("mogwai.kcore").project("name","coreValue").by("name").by("coreValue")`)).map(unmap) as any[])
  ...
  test('every vertex decorated; order().by(coreValue) composes', async () => {
    ...
    expect(await run(store, `g.V().call("mogwai.kcore").has("coreValue").count()`)).toEqual([4]);
    expect(await run(store, `g.V().call("mogwai.kcore").order().by("coreValue", Order.desc).by("name").limit(1).values("name")`))
      .toEqual(['x']);
  });
});
