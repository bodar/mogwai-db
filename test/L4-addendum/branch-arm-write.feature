Feature: mogwai addendum — a WRITE inside a SINGLE-arm branch
  ...
  #
  # A MULTI-arm branch with a write, and choose/coalesce writes (always ≥2 arms), DECLINE cleanly — effects
  # run before the result query, so a sibling arm would observe the write and answer a post-write state the
  # reference does not. The @Unsupported scenarios below pin that fail-closed refusal.
  ...
  # A write in a MULTI-arm branch declines (a sibling arm would observe the mutation).
  @Unsupported
  @gap:branch-arm-write
