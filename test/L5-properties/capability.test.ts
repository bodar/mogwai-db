...
describe('L5 capability ratchet', () => {
  test('shape-legal generated transitions either execute or fail closed', () => {
    ...
    for (const generated of samples) {
      const outcome = outcomeOf(seeded(MODERN_SEED), generated.query, ALL_GENERIC);
      if (outcome.kind === 'rows') { ran++; continue; }
      deferred++;
      if (RAW_FAILURE.test(outcome.message)) {
        const expected = KNOWN_RAW_WITNESSES.get(generated.query);
        if (expected === outcome.message) seenKnown.add(generated.query);
        else raw.push(`${generated.query}\n    ${outcome.message}`);
      }
    }
    console.log(`L5 capability: ${samples.length} witnesses — ${ran} ran, ${deferred} declared deferrals`);
    expect(raw).toEqual([]);
