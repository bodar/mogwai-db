...
// EMPTY, and that is the state a ratchet is supposed to reach. Its one entry recorded a defect in
// the LEGACY generic child-existence lowering — a route that no longer exists, so the divergence went
// with it. Kept as a live list rather than deleted: the next per-switch divergence L5 draws belongs
