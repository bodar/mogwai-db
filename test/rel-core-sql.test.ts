...
describe('RelIR relational-core SQL', () => {
  test('pins ten representative L2 relational cores byte-for-byte', () => {
    ...
    const f = filter({ id: relId('f'), input: n, layout, type: { cols }, pred: { kind: 'binary', op: '=', left: col(n.id, 'name'), right: lit('marko', 'text') } });
    ...
    const m = scan('m');
    const j = join({ id: relId('j'), left: n, right: m, join: 'inner', on: { kind: 'binary', op: '=', left: col(n.id, 'id'), right: col(m.id, 'id') }, layout, type: { cols } });
